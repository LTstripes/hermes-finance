...
- `/` is now the accepted primary UI v2 entry; `/v2` remains valid and historical legacy routes/editors remain available;
- UI v2 default switch passed comparative review and exact-SHA Owner UAT before canonical integration;
- Windows launcher/runtime remains a prepared-runtime shell around the proven external OPS01/OPS02/OPS03 operations rather than a monolithic updater.

### Fixed

- restore results distinguish confirmed negative outcomes from possibly-mutated ambiguity via machine-readable `restore_outcome_ambiguous`;
- ambiguous restore UI claims neither success nor failure, refreshes shared reads and never blind-retries;
- cleanup cannot overwrite an already-classified ambiguous restore outcome.
