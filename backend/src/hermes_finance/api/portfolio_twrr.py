...
class PortfolioTwrrResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
def _response(result: PortfolioTwrrResult) -> PortfolioTwrrResponse:
    return PortfolioTwrrResponse(
        ...
        annualized=False,
        period=TwrrPeriodOut(start_date=result.start_date, end_date=result.end_date),
...
@router.get("/twrr", response_model=PortfolioTwrrResponse)
def read_portfolio_twrr(
