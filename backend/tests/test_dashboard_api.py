...
def test_summary_and_dashboard_happy_path(client: TestClient) -> None:
    ...
    assert dash["summary"]["forecast"]["breakdown"]["expected_deposit_interest"] == _rub("12000.00")
    ...
    assert dash["summary"]["passive_income_average_months"] == 1
    assert dash["summary"]["passive_income_average_complete"] is False
