...
- 2026-08-09 — выполнен `E12` (DeepSeek V4 Flash primary + GLM 5.2/Nvidia NIM reviewer): closed-month capital chart — backend `historical_series` фильтруется по закрытым месяцам (draft исключены; series растёт при закрытии), Recharts 3.10.1 по MASTER_SPEC §10, `CapitalChart` с tooltip/₽/разрывом линии без интерполяции/empty state на DashboardPage; frontend 50, backend 423, lint/format/build green; `E13` не начат.
