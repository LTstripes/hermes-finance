...
> **Исторический MVP backlog:** `docs/history/HERMES_TASKS.md`
...
`docs/history/HERMES_TASKS.md` сохраняется как исторический пошаговый backlog строительства MVP. Во время работы над 0.2 старые разделы H/I/J/K рассматривались только как legacy roadmap и не входили в релиз автоматически; нужные идеи переносились владельцем в этот release record отдельными `R02-*` task-cards.
