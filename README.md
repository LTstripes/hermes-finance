...
Версия этого release candidate — **0.4.0**.
...
5. При необходимости внесите ожидаемые выплаты, расходы, savings, долги, недвижимость, ИИС и комментарии.
...
В **0.4.0 календарь заполняется вручную** через «Новая ожидаемая выплата». Приложение пока не генерирует автоматически купонный/дивидендный/погашательный календарь из текущих позиций и не загружает расписание MOEX. Автоматическое наполнение требует отдельного контракта источника, refresh/version semantics и защиты ручных записей от дублей/перезаписи.
...
- котировки T-Invest только после явного preview/apply владельца; фонового обновления нет; MOEX не является production fallback;
- автоматического календаря будущих выплат нет — expected flows вводятся вручную;
...
- [`docs/releases/0.4.0.md`](docs/releases/0.4.0.md) — current release backlog/checkpoint;
