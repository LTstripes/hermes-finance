...
Версия этого release candidate — **0.6.0**. Последний опубликованный тег остаётся `v0.5.0`; Gate C не создаёт `v0.6.0` и не публикует GitHub Release.
...
В **0.6.0** календарь по-прежнему объединяет ручные ожидаемые выплаты и уже применённые события T-Invest. Alfa statement import — отдельный явный путь фактических выплат, не автозаполнение календаря.
...
- [`docs/releases/0.6.0.md`](docs/releases/0.6.0.md) — активный 0.6 release candidate;
- [`docs/release-notes-0.6.0.md`](docs/release-notes-0.6.0.md) — публичные notes для позднего guarded release helper.
