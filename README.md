...
Проект развивается небольшими последовательными задачами. Сейчас доступны минимальный FastAPI backend, React frontend с проверкой подключения, локальная SQLite-конфигурация через SQLAlchemy 2 и Alembic service baseline; доменная схема данных и финансовая логика ещё не реализованы.
...
Alembic хранит версионируемую историю схемы. В B02 создана служебная baseline-миграция без доменных таблиц; первые таблицы появятся отдельными задачами backlog.

```bash
cd backend
uv run alembic upgrade head      # обновить пустую или существующую локальную БД
uv run alembic current           # показать применённую revision
uv run alembic downgrade base    # вернуться к состоянию без revision
```

Команды используют `HERMES_FINANCE_DATABASE_PATH` (или путь по умолчанию `data/finance.db` от корня репозитория). Для безопасной проверки задайте путь к синтетической временной БД; не применяйте downgrade к пользовательской базе без backup.
