...
The script runs the automated safety harness (75 checks including #279 identity/CTA/config/setup and #298 Stable upgrade regressions) and publishes a self-contained single-file `win-x64` executable to `artifacts\win-x64\HermesFinance.Launcher.exe`. Build artifacts are ignored and must not be committed.
...
Use it to inspect the Stable-ready ( `Release v0.8.2 · production` ), Preview-UNRELEASED and opt-in diagnostics states on Windows, then close the window normally.
