...
try {
    ...
    try {
        Invoke-NativeStep -Label "Linting backend" -Action {
    ...
    try {
        Invoke-NativeStep -Label "Linting frontend" -Action {
            & $npm run lint
        }
    }
    finally {
    ...
    Write-Host "Backend and frontend lint checks passed." -ForegroundColor Green
}
catch {
