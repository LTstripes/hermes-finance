...
# and /api/health reports version 0.6.3.
...
try {
    ...
    if ($healthBody.status -ne "ok" -or $healthBody.version -ne "0.6.3") {
        throw "Expected health {status=ok, version=0.6.3}, got '$($healthResponse.Content)'."
    }
    Write-Host "R06-10 Windows smoke: /api/health reports 0.6.3." -ForegroundColor Green
